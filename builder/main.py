...
# Slightly trimmed copy of platformio/platform-nxplpc/builder/main.py: drops
# the zephyr-pre SConscript branch (we don't bundle zephyr); keeps the
# LPC vector-checksum post-action since LPC8xx bootloaders require it.
...
def add_image_checksum(source, target, env):
    # NXP LPC parts store a vector-table checksum at offset 0x1C; the
    # bootloader uses it to identify a valid image. Compute the two's
    # complement of the sum of vector entries 0..6.
